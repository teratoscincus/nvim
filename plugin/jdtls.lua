...
local function get_jdtls_paths()
  ...
  path.runtimes = {
    ...
    {
      name = "JavaSE-1.8",
      path = vim.fn.expand("~/.jenv/versions/1.8.0.322/"),
    },
    {
      name = "JavaSE-17",
      path = vim.fn.expand("~/.jenv/versions/17.0.7/"),
    },
  }
...
local function jdtls_on_attach(client, bufnr)
  ...
  vim.keymap.set("n", "ev", "<cmd>lua require('jdtls').extract_variable()<cr>", opts)          -- (E)ctract (v)ariable
  vim.keymap.set("n", "ec", "<cmd>lua require('jdtls').extract_constant()<cr>", opts)          -- (E)ctract (c)onstant

  vim.keymap.set("x", "ev", "<esc><cmd>lua require('jdtls').extract_variable(true)<cr>", opts) -- (E)ctract (v)ariable
  vim.keymap.set("x", "ec", "<esc><cmd>lua require('jdtls').extract_constant(true)<cr>", opts) -- (E)ctract (c)onstant
  vim.keymap.set("x", "em", "<esc><Cmd>lua require('jdtls').extract_method(true)<cr>", opts)   -- (E)ctract (m)ethod
end
